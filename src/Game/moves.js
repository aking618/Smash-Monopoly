    ...
    switch (ctx.currentPlayer) {
        case "0":
            ...
            if (IsTileTax(G.player1Pos)) {
                G.showEffectPopup = true;
                G.effectType = POPUP_TYPES.TAX;
            }

            if (IsTileChance(G.player1Pos)) {
                G.showEffectPopup = true;
                G.effectType = POPUP_TYPES.CHANCE;
            }

            if (IsTileCommunityChest(G.player1Pos)) {
                G.showEffectPopup = true;
                G.effectType = POPUP_TYPES.COMMUNITY_CHEST;
            }
        ...
        case "1":
            ...
            if (IsTileChance(G.player2Pos)) {
                G.showEffectPopup = true;
                G.effectType = POPUP_TYPES.CHANCE;
            }

            if (IsTileCommunityChest(G.player2Pos)) {
                G.showEffectPopup = true;
                G.effectType = POPUP_TYPES.COMMUNITY_CHEST;
            }
...
export function acceptEffectAction({ G, ctx, events }) {
    G.showEffectPopup = false;
    G.effectType = POPUP_TYPES.NONE;
